...
def Create_Task_Window(listbox):
    ...
    def Save_Task():
        ...
        if not assignee or not assignee.replace(" ", "").isalpha():
            messagebox.showerror("Ошибка", "Исполнитель должен быть введён в формате ФИО")
            assignee_entry.delete(0, END)
            return
        
        # Проверка правильности написания даты дедлайна
        if not deadline or not re.match(r'\d{1,2}\.\d{1,2}\.\d{4}', deadline):
            messagebox.showerror("Ошибка", "Дедлайн должен быть указан в формате день.месяц.год (например, 31.12.2024)")
